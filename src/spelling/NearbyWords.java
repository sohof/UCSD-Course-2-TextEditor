...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		// for each k+1 positions where a new char can be inserted in k long string
		for (int i = 0; i <= s.length(); ++i) {

			for (int charCode = (int)'a'; charCode <=(int)'z'; charCode++) { // cast char 'a' to int to get ascci int repr. 1 - z
				StringBuilder sb = new StringBuilder(s);
				sb.insert(i, (char) charCode); // cast charcode int rep. back to a char
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (!queue.isEmpty() && retList.size() <= numSuggestions) {
			...
			// get a list of neighbors using distanceOne (strings one mutation away from curr)
